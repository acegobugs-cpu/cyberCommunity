...
| learn | ✅ **L1 content model**: `courses(status DRAFT\|PUBLISHED\|ARCHIVED, slug, difficulty, tags[], estimated_minutes cache) → modules → lessons(type READING\|VIDEO\|QUIZ\|LAB\|PROJECT)`. GraphQL: `courses(filter)`, `course(slug)`, `courseById`, `lesson`; author `upsertCourse/Module/Lesson`, `reorderModules/Lessons(orderedIds)`, `publishCourse`, `archiveCourse`, `deleteModule/Lesson`. Learners see PUBLISHED only (repo-level filter); `@BatchMapping` for modules/lessons. Auth via `Policies.LEARNER` / `CONTENT_AUTHOR`; 15 integration tests. Plan 01 L1 done → next L2 (enrollment & progress) |
...
5. **Frontend never calls the gateway directly.** Browser → `/api/*` route handler → `forwardToGateway` / `gatewayFetch` (`src/lib/gateway.ts`). Server components use `src/lib/data/*` + `getSessionToken()`; client uses `src/lib/api.ts` + `useAuth()`. **Learn has one door:** `POST /api/learn/graphql`; server code uses `lib/data/learn.ts`, client uses `lib/learn/client.ts`; operation strings in `src/graphql/learn-documents.ts`, types generated into `src/graphql/generated.ts` by `npm run codegen` (schema = `learn/src/main/resources/graphql/schema.graphqls`). Learn pages live in `src/app/sites/learn/**` and are served on the `learn.` host (`/`, `/courses/[slug]`, `/courses/[slug]/lessons/[id]`, `/admin`, `/admin/courses/[id]`).
