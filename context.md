...
| learn | ✅ GraphQL `courses`, `course`, `createCourse`, `createLesson`; **authorized** (`Policies.LEARNER` reads, `CONTENT_AUTHOR` = AUTHOR|ADMIN writes) via identity `member/check?serviceName=learn`; `JwtFilter` rejects missing/malformed `X-User-Id` with 401; `QueryExceptionHandler` maps domain exceptions → GraphQL `ErrorType` + `extensions.correlationId`; GraphiQL on locally, off in docker; 9 integration tests (Testcontainers + MockWebServer identity). Plan 01 L0 done → next L1 |
