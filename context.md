...
| learn | ✅ **L1 content + L2 progress**: `paths(status DRAFT\|PUBLISHED\|ARCHIVED, slug, difficulty, tags[], estimated_minutes cache) → modules → lessons(type READING\|VIDEO\|QUIZ\|LAB\|PROJECT)` ("courses" were renamed **paths**). Progress: `enrollments(status ENROLLED\|COMPLETED\|DROPPED, progress)`, `module_progress(status NOT_STARTED\|IN_PROGRESS\|COMPLETED\|DROPPED)`, `completed_lessons`; progress is **derived in Postgres** by `learn.recompute_progress()` (trigger on `completed_lessons`; `ProgressRepo.recomputeAll` when lessons change). GraphQL: `paths(filter)`, `path(slug)`, `pathById`, `lesson`, `myEnrollments`, `Path.enrollment{status,progress,nextLessonId}`, `Module.myProgress`, `Lesson.completed`; learner `enroll/dropPath/startModule/dropModule/completeLesson` (idempotent, implicit enroll); author `upsertPath/Module/Lesson`, `reorderModules/Lessons`, `publishPath`, `archivePath`, `deleteModule/Lesson`. Learners see PUBLISHED only; `@BatchMapping` for modules/lessons. Auth via `Policies.LEARNER` / `CONTENT_AUTHOR`; 22 integration tests. **V1 was rewritten (courses→paths) — dev DB needs `DROP SCHEMA learn CASCADE` + re-migrate.** Next: L3 roadmaps / L4 quizzes |
